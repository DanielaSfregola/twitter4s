...
trait TwitterStatusClient extends StreamingOAuthClient with Configurations with ActorContextExtractor {
  ...
  /** Starts a statuses connection from Twitter's public API, filtered with the 'follow', 'track' and 'location' parameters.
    ...
    * <a href="https://dev.twitter.com/statuses/reference/post/statuses/filter" target="_blank">
    *   https://dev.twitter.com/statuses/reference/post/statuses/filter</a>.
  ...
  /** Starts a statuses connection from Twitter's public API, which is a a small random sample of all public statuses.
    ...
    * If the function wasn't invoked from an actor, you can specify which actor will receive the updates with the implicit variable `self`.
    * For more information see
    * <a href="https://dev.twitter.com/statuses/reference/get/statuses/sample" target="_blank">
    *   https://dev.twitter.com/statuses/reference/get/statuses/sample</a>.
    * Note: delimited is, for now, not supported
    *
    * @param stall_warnings : Specifies whether stall warnings (`WarningMessage`) should be delivered as part of the updates.
    */
