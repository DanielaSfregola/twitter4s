...
class ActorStreamingClientSpec(implicit ee: ExecutionEnv) extends Specification with ClientSpec with LazyLogging with JsonSupport {
  ...
  "StreamingClient" should {
    {
      ...
      "receive a disconnect message from the mocked connection stream" in {
        ...
        val sampleRequest = HttpRequest(HttpMethods.POST, s"$statusUrl/filter.json", entity = HttpEntity(ContentTypes.`application/json`, Serialization.write(msg) + ByteString("\r\n"))
        )

        val dd = Serialization.write(msg)


        val actorRef = system.actorOf(ActorStreamingClientWithFakeHttp.props(ct, at, sampleRequest))
...
//        val streamSource = Await.result(streamSourceFuture, 20 seconds)
          ...
          streamResult must be(msg).await(0, 5 seconds)
//        0 must be 0
