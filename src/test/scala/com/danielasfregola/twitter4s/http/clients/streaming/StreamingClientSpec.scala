  ...
  "StreamingClient" should {
    ...
    "parse a stream of data and output the corresponding entities of type" in {
      ...
      "UserEnvelopTweetStringified" in new StreamingClientSpecContext {
        val response = buildResponse("/twitter/streaming/user_envelop_tweet_stringified.json")
        processBody(response, killSwitch)(redirectMessages)

        val messages = readStreamUpdatesAs[UserEnvelopTweetStringified]("/fixtures/streaming/user_envelop_tweet_stringified.json")
        transport.expectMsgAllOf(messages: _*)
      }

      "UserEnvelopDirectMessageStringified" in new StreamingClientSpecContext {
        val response = buildResponse("/twitter/streaming/user_envelop_direct_message_stringified.json")
        processBody(response, killSwitch)(redirectMessages)

        val messages = readStreamUpdatesAs[UserEnvelopDirectMessageStringified]("/fixtures/streaming/user_envelop_direct_message_stringified.json")
        transport.expectMsgAllOf(messages: _*)
      }

      "UserEnvelopTwitterListEventStringified" in new StreamingClientSpecContext {
        val response = buildResponse("/twitter/streaming/user_envelop_twitter_list_event_stringified.json")
        processBody(response, killSwitch)(redirectMessages)

        val messages = readStreamUpdatesAs[UserEnvelopTwitterListEventStringified]("/fixtures/streaming/user_envelop_twitter_list_event_stringified.json")
        transport.expectMsgAllOf(messages: _*)
      }

      "UserEnvelopTweetEventStringified" in new StreamingClientSpecContext {
        val response = buildResponse("/twitter/streaming/user_envelop_tweet_event_stringified.json")
        processBody(response, killSwitch)(redirectMessages)

        val messages = readStreamUpdatesAs[UserEnvelopTweetEventStringified]("/fixtures/streaming/user_envelop_tweet_event_stringified.json")
        transport.expectMsgAllOf(messages: _*)
      }

      "UserEnvelopSimpleEventStringified" in new StreamingClientSpecContext {
        val response = buildResponse("/twitter/streaming/user_envelop_simple_event_stringified.json")
        processBody(response, killSwitch)(redirectMessages)

        val messages = readStreamUpdatesAs[UserEnvelopSimpleEventStringified]("/fixtures/streaming/user_envelop_simple_event_stringified.json")
        transport.expectMsgAllOf(messages: _*)
      }

      "UserEnvelopWarningMessageStringified" in new StreamingClientSpecContext {
        val response = buildResponse("/twitter/streaming/user_envelop_warning_message_stringified.json")
        processBody(response, killSwitch)(redirectMessages)

        val messages = readStreamUpdatesAs[UserEnvelopWarningMessageStringified]("/fixtures/streaming/user_envelop_warning_message_stringified.json")
        transport.expectMsgAllOf(messages: _*)
      }
