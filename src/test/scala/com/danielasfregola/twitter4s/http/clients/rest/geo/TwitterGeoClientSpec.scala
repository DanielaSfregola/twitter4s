...
class TwitterGeoClientSpec extends ClientSpec {
  ...
  "Twitter Geo Client" should {
    ...
    "get a geo place by id" in new TwitterGeoClientSpecContext {
      ...
      val result: RatedData[GeoPlace] = when(geoPlace(placeId)).expectRequest { request =>
        request.method === HttpMethods.GET
        request.uri.endpoint === s"https://api.twitter.com/1.1/geo/id/$placeId.json"
      }.respondWithRated("/twitter/rest/geo/geo_place.json").await
    ...
    "perform a reverse geocode search" in new TwitterGeoClientSpecContext {
      val result: RatedData[GeoSearch] = when(reverseGeocode(-122.42284884, 37.76893497)).expectRequest { request =>
        request.method === HttpMethods.GET
      ...
      }.respondWithRated("/twitter/rest/geo/reverse_geocode.json").await
      result.rate_limit === rateLimit
    ...
    "search a geo place" in new TwitterGeoClientSpecContext {
      val result: RatedData[GeoSearch] = when(searchGeoPlace("Creazzo")).expectRequest { request =>
        request.method === HttpMethods.GET
        request.uri.endpoint === s"https://api.twitter.com/1.1/geo/search.json"
        request.uri.queryString() === Some("query=Creazzo")
      }.respondWithRated("/twitter/rest/geo/search.json").await
      result.rate_limit === rateLimit
    ...
    "perform an advanced search of a geo place" in new TwitterGeoClientSpecContext {
      val result: RatedData[GeoSearch] = when(advancedSearchGeoPlace(query = Some("Creazzo"), street_address = Some("Via Giotto 15"))).expectRequest { request =>
        request.method === HttpMethods.GET
        request.uri.endpoint === s"https://api.twitter.com/1.1/geo/search.json"
        request.uri.queryString() === Some("attribute:street_address=Via+Giotto+15&query=Creazzo")
      }.respondWithRated("/twitter/rest/geo/advanced_search.json").await
      result.rate_limit === rateLimit
