...
class TwitterFollowerClientSpec extends ClientSpec {
  ...
  "Twitter Follower Client" should {
    ...
    "get followers ids of a specific user by id" in new TwitterFollowerClientSpecContext {
      val result: RatedData[UserIds] = when(followerIdsForUserId(2911461333L)).expectRequest { request =>
      ...
      }.respondWithRated("/twitter/rest/followers/followers_ids.json").await
    ...
    "get followers ids of a specific user by name" in new TwitterFollowerClientSpecContext {
      val result: RatedData[UserIds] = when(followerIdsForUser("DanielaSfregola")).expectRequest { request =>
        request.method === HttpMethods.GET
        request.uri.endpoint === "https://api.twitter.com/1.1/followers/ids.json"
    ...
    "get followers stringified ids of a specific user by id" in new TwitterFollowerClientSpecContext {
      val result: RatedData[UserStringifiedIds] = when(followerStringifiedIdsForUserId(2911461333L)).expectRequest { request =>
        request.method === HttpMethods.GET
        request.uri.endpoint === "https://api.twitter.com/1.1/followers/ids.json"
        request.uri.queryString() === Some("count=5000&cursor=-1&stringify_ids=true&user_id=2911461333")
      }.respondWithRated("/twitter/rest/followers/followers_ids_stringified.json").await
      result.rate_limit === rateLimit
    ...
    "get followers stringified ids of a specific user by name" in new TwitterFollowerClientSpecContext {
      val result: RatedData[UserStringifiedIds] = when(followersStringifiedIdsForUser("DanielaSfregola")).expectRequest { request =>
        request.method === HttpMethods.GET
        request.uri.endpoint === "https://api.twitter.com/1.1/followers/ids.json"
    ...
    "get followers of a specific user" in new TwitterFollowerClientSpecContext {
      val result: RatedData[Users] = when(followersForUser("DanielaSfregola")).expectRequest { request =>
        request.method === HttpMethods.GET
      ...
      }.respondWithRated("/twitter/rest/followers/followers.json").await
      result.rate_limit === rateLimit
    ...
    "get followers of a specific user by user id" in new TwitterFollowerClientSpecContext {
      val result: RatedData[Users] = when(followersForUserId(2911461333L)).expectRequest { request =>
        request.method === HttpMethods.GET
        request.uri.endpoint === "https://api.twitter.com/1.1/followers/list.json"
