...
class MentionsParametersSpec extends SpecificationLike {
  ...
  "MentionsParameters" should {
    "correctly represents each field as the respective request parameter" in {
      MentionsParameters(
        count = 20, since_id = Some(100L), max_id = Some(200L), trim_user = true, contributor_details = true,
    ...
    "doesn't provide request parameter if the respective optional field is empty" in {
      MentionsParameters(
        count = 20, since_id = None, max_id = None, trim_user = false, contributor_details = false,
