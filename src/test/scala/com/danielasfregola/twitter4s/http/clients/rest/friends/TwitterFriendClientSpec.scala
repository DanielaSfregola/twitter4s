...
class TwitterFriendClientSpec extends ClientSpec {
  ...
  "Twitter Friend Client" should {
    ...
    "get friends ids of a specific user by id" in new TwitterFriendClientSpecContext {
      val result: RatedData[UserIds] = when(friendIdsForUserId(2911461333L)).expectRequest { request =>
      ...
      }.respondWithRated("/twitter/rest/friends/friends_ids.json").await
    ...
    "get friends ids of a specific user by name" in new TwitterFriendClientSpecContext {
      val result: RatedData[UserIds] = when(friendIdsForUser("DanielaSfregola")).expectRequest { request =>
        request.method === HttpMethods.GET
        request.uri.endpoint === "https://api.twitter.com/1.1/friends/ids.json"
    ...
    "get friends stringified ids of a specific user by id" in new TwitterFriendClientSpecContext {
      val result: RatedData[UserStringifiedIds] = when(friendStringifiedIdsForUserId(2911461333L)).expectRequest { request =>
        request.method === HttpMethods.GET
        request.uri.endpoint === "https://api.twitter.com/1.1/friends/ids.json"
        request.uri.queryString() === Some("count=5000&cursor=-1&stringify_ids=true&user_id=2911461333")
      }.respondWithRated("/twitter/rest/friends/friends_ids_stringified.json").await
      result.rate_limit === rateLimit
    ...
    "get friends stringified ids of a specific user by name" in new TwitterFriendClientSpecContext {
      val result: RatedData[UserStringifiedIds] = when(friendStringifiedIdsForUser("DanielaSfregola")).expectRequest { request =>
        request.method === HttpMethods.GET
        request.uri.endpoint === "https://api.twitter.com/1.1/friends/ids.json"
    ...
    "get friends of a user" in new TwitterFriendClientSpecContext {
      val result: RatedData[Users] = when(friendsForUser("DanielaSfregola", count = 10)).expectRequest { request =>
        request.method === HttpMethods.GET
      ...
      }.respondWithRated("/twitter/rest/friends/users.json").await
      result.rate_limit === rateLimit
    ...
    "get friends of a user by user id" in new TwitterFriendClientSpecContext {
      val result: RatedData[Users] = when(friendsForUserId(2911461333L, count = 10)).expectRequest { request =>
        request.method === HttpMethods.GET
        request.uri.endpoint === "https://api.twitter.com/1.1/friends/list.json"
